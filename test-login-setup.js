...
function loadEnvFile() {
    try {
        ...
        logError('Could not read .env file. Make sure it exists in the project root.');
        return null;
    }
}
...
async function testSupabaseConnection(supabaseUrl, supabaseKey) {
    try {
        ...
        if (error && error.message.includes('Invalid API key')) {
            logError('Invalid Supabase API key. Please check your credentials.');
            return false;
        }
...
async function runTests() {
    ...
    for (const varName of requiredVars) {
        if (!env[varName] || env[varName].includes('your-') || env[varName].includes('placeholder')) {
    ...
    if (!envValid) {
        logError('Environment configuration is incomplete. Please update your .env file.');
    ...
    const supabase = await testSupabaseConnection(env.VITE_SUPABASE_URL, env.VITE_SUPABASE_ANON_KEY);
    ...
    if (!tablesExist) {
        ...
        logInfo('Please create the required tables as described in LOGIN_SETUP_GUIDE.md');
    }
    ...
    if (tablesExist) {
        ...
        log('1. Follow the database setup steps in LOGIN_SETUP_GUIDE.md', 'blue');
