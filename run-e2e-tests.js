...
async function checkTestFiles() {
    ...
    for (const [testName, description] of Object.entries(testSuites)) {
        ...
        console.log(
            `${exists ? '✅' : '❌'} ${testName}.spec.ts - ${description}`
        );
...
async function checkPlaywrightConfig() {
    ...
    if (exists) {
        ...
        const checks = [
            {
                key: 'testDir',
                pattern: /testDir:\s*['"]\.\/tests['"]/,
                description: 'Test directory configured',
            },
            {
                key: 'baseURL',
                pattern: /baseURL:\s*['"]http:\/\/localhost:5173['"]/,
                description: 'Base URL configured',
            },
            {
                key: 'projects',
                pattern: /projects:\s*\[/,
                description: 'Browser projects configured',
            },
            {
                key: 'webServer',
                pattern: /webServer:\s*{/,
                description: 'Web server configured',
            },
        ];
...
async function runE2ETests() {
    try {
        ...
        const testCommands = [
            ...
            {
                cmd: 'npm',
                args: ['run', 'test:e2e:organizations', '--', '--list'],
            ...
            {
                cmd: 'npm',
                args: ['run', 'test:e2e:organizations:crud'],
                description: 'Run CRUD workflow tests',
            },
        ];
        ...
        Object.entries(testSuites).forEach(([name, description]) => {
            console.log(
                `  • npm run test:e2e:organizations:${name.replace('-', ':')}`
            );
        ...
        console.log(
            '  npm run test:e2e:organizations     # Run all organization tests'
        );
