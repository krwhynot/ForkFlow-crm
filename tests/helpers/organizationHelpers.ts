...
export class OrganizationHelpers {
    ...
    async fillForm(data: OrganizationFormData) {
        ...
        if (data.priorityId !== undefined)
            await this.selectOption('priorityId', String(data.priorityId));
        if (data.segmentId !== undefined)
            await this.selectOption('segmentId', String(data.segmentId));
    ...
    async submit() {
        await this.page.click(
            'button[type="submit"], .MuiButton-contained:has-text("Save")'
        );
        await this.page.waitForLoadState('networkidle');
    }
    ...
    async selectOption(field: string, value: string) {
        ...
        await this.page.click(
            `[data-value="${value}"], li:has-text("${value}")`
        );
    }
    ...
    async createTestOrg(data: OrganizationFormData = {}): Promise<string> {
        ...
        await this.page.waitForSelector(
            '.MuiAlert-root, .ra-notification, .organization-details'
        );
    ...
    async waitForGps() {
        await this.page.waitForSelector('.gps-success, .MuiAlert-success', {
            timeout: 10000,
        });
    }
    ...
    async expectMobileLayout() {
        if (await this.isMobile()) {
            await expect(
                this.page.locator('.mobile-layout, .MuiContainer-maxWidthSm')
            ).toBeVisible();
        }
    }
    ...
    async checkA11yForm() {
        const inputs = await this.page
            ...
            .count();
        const labelsAndAria = await this.page
            .locator(
                'input[aria-label], input[aria-labelledby], input + label, label input'
            )
            .count();
    ...
    async searchOrganizations(query: string) {
        ...
        await this.page.press(
            'input[name="q"], .ra-search-input input',
            'Enter'
        );
        await this.page.waitForLoadState('networkidle');
    }
    ...
    async getOrganizationRowCount(): Promise<number> {
        await this.page.waitForSelector(
            '.MuiDataGrid-row, .ra-datagrid tbody tr'
        );
        const rows = await this.page
            .locator('.MuiDataGrid-row, .ra-datagrid tbody tr')
            .count();
        return rows;
    }
    ...
    async clickOrganizationRow(organizationName: string) {
        await this.page.click(
            `tr:has-text("${organizationName}"), .MuiDataGrid-row:has-text("${organizationName}")`
        );
    }
    ...
    async expectValidationError(fieldName: string, errorMessage?: string) {
        ...
        if (errorMessage) {
            await expect(this.page.locator(errorSelector)).toContainText(
                errorMessage
            );
        }
    }
    ...
    async expectOrganizationDetails(expectedData: Partial<Organization>) {
        if (expectedData.name) {
            await expect(
                this.page.locator('h1, .organization-name')
            ).toContainText(expectedData.name);
        }

        if (expectedData.address) {
            await expect(
                this.page.locator('.address, .organization-address')
            ).toContainText(expectedData.address);
        }

        if (expectedData.phone) {
            await expect(
                this.page.locator('.phone, .organization-phone')
            ).toContainText(expectedData.phone);
        }

        if (expectedData.website) {
            await expect(
                this.page.locator('a[href*="http"], .website-link')
            ).toContainText(expectedData.website);
        }
    }
...
export async function expectSuccessNotification(page: Page, message?: string) {
    await expect(
        page.locator('.MuiAlert-success, .ra-notification-success')
    ).toBeVisible();
    if (message) {
        await expect(
            page.locator('.MuiAlert-success, .ra-notification-success')
...
export async function expectErrorNotification(page: Page, message?: string) {
    await expect(
        page.locator('.MuiAlert-error, .ra-notification-error')
    ).toBeVisible();
    if (message) {
        await expect(
            page.locator('.MuiAlert-error, .ra-notification-error')
