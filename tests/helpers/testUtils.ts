...
export class TestUtils {
  ...
  async waitForAppReady(): Promise<void> {
    ...
    await this.page.waitForSelector('[data-testid="app-ready"], .main-content, .ra-layout-content', { timeout: 20000 });
  }
   ...
   */
  async login(email: string = 'demo@forkflow.com', password: string = 'demo123'): Promise<void> {
    await this.page.goto('/login');
    await this.page.fill('input[name="email"], input[type="email"], #email', email);
    await this.page.fill('input[name="password"], input[type="password"], #password', password);
    await this.page.click('button[type="submit"], .login-button, [data-testid="login-submit"]');
