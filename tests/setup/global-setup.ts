...
async function globalSetup(config: FullConfig) {
  ...
  try {
    ...
    console.log('📡 Connecting to application at', config.projects[0].use.baseURL);
    ...
    try {
      await page.waitForSelector('.MuiToolbar-root, .ra-layout, [data-testid="dashboard"]', { 
        timeout: 30000 
      });
      console.log('✅ Application loaded successfully');
    } catch (error) {
      console.log('⚠️ Application loading timeout (this may be normal in test setup)');
    }
