...
test.describe('Organization CRUD Workflow', () => {
  ...
  test.describe('Read Organization', () => {
    ...
    test('should display computed statistics', async () => {
      ...
      await expect(orgHelpers.page.locator('.organization-stats, .stats-section')).toBeVisible();
    });
    ...
    test('should show related contacts and deals', async () => {
      ...
      await expect(orgHelpers.page.locator('.contacts-section, .related-contacts')).toBeVisible();
      await expect(orgHelpers.page.locator('.deals-section, .opportunities-section')).toBeVisible();
    });
  });
  ...
  test.describe('Update Organization', () => {
    test('should update organization information', async () => {
      ...
      // Navigate to edit
      const orgId = await orgHelpers.page.url().match(/\/organizations\/(\d+)/)?.[1];
      ...
      await expect(orgHelpers.page).toHaveURL(/\/organizations\/\d+/);
      await expect(orgHelpers.page.locator('.success-notification')).toBeVisible();
    ...
    test('should validate updates', async () => {
      ...
      // Should not redirect
      await expect(orgHelpers.page).toHaveURL(/\/organizations\/\d+\/edit/);
    });
  ...
  test.describe('Delete Organization', () => {
    test('should delete organization', async () => {
      ...
      // Click delete button
      await orgHelpers.page.click('.delete-button, button:has-text("Delete")');

      // Confirm deletion in modal
      await orgHelpers.page.click('.MuiDialog-root button:has-text("Delete"), .confirm-delete');
    ...
    test('should require confirmation for deletion', async () => {
      ...
      await expect(orgHelpers.page.locator('.MuiDialog-root, .confirmation-modal')).toBeVisible();
      await expect(orgHelpers.page.locator('.MuiDialog-root')).toContainText('Are you sure');

      // Cancel should close dialog
      await orgHelpers.page.click('.MuiDialog-root button:has-text("Cancel"), .cancel-delete');
      await expect(orgHelpers.page.locator('.MuiDialog-root')).toBeHidden();
    ...
    test('should handle deletion errors gracefully', async () => {
      ...
      await expect(orgHelpers.page.locator('.error-notification')).toBeVisible();
  ...
  test.describe('Bulk Operations', () => {
    test('should handle bulk updates', async () => {
      ...
      const checkboxes = orgHelpers.page.locator('.MuiCheckbox-root, .selection-checkbox');
      await checkboxes.first().click();
      await checkboxes.nth(1).click();
      await checkboxes.nth(2).click();

      // Perform bulk action
      await orgHelpers.page.click('.bulk-actions button:has-text("Update"), .bulk-update-button');

      // Should show bulk update form or modal
      await expect(orgHelpers.page.locator('.bulk-update-modal, .MuiDialog-root')).toBeVisible();
    });
    ...
    test('should export organization data', async () => {
      ...
      // Click export button
      await orgHelpers.page.click('.export-button, button:has-text("Export")');

      // Should trigger download (we can't easily test the actual file download in Playwright)
      // But we can verify the button works
      await expect(orgHelpers.page.locator('.export-button')).toBeVisible();
    });
  });
