...
test.describe('Authentication - Sign Up', () => {
    ...
    test('should show activation message after successful sign up', async ({
        page,
    }) => {
        const auth = new AuthHelpers(page);
        await auth.gotoSignUp();
        ...
        await auth.submitSignUp();
        await expect(
            page.locator('[data-testid="activation-message"]')
        ).toBeVisible();
    });
    ...
    test('should load the sign-up page without JS or console errors', async ({
        page,
    }) => {
        const errors: string[] = [];
