...
import { validateBasicInfo, validateContactDetails, validateBusinessDetails } from './validation';
...
export const useRealTimeValidation = (
    ...
    // Validation state
    const [validationState, setValidationState] = useState<RealTimeValidationState>({
        isValidating: false,
        validationResults: {},
    ...
    const checkDuplicate = useCallback(async (name: string): Promise<boolean> => {
    ...
    const checkBusinessDuplicate = useCallback(async (
        name: string,
        address?: string,
        phone?: string
        ...
        try {
            ...
            if (address && address.trim().length > 0) {
                ...
                const addressResult = await dataProvider.getList('organizations', {
                    pagination: { page: 1, perPage: 1 },
                    sort: { field: 'id', order: 'DESC' },
            ...
            if (phone && phone.trim().length > 0) {
                ...
                if (phoneDigits.length >= 10) {
                    ...
                    const phoneResult = await dataProvider.getList('organizations', {
                        pagination: { page: 1, perPage: 1 },
                        sort: { field: 'id', order: 'DESC' },
                        filter: phoneFilter,
                    });
                    ...
                    if (phoneResult.data.length > 0) {
                        return { 
                            isDuplicate: true, 
                            matchReason: 'Similar name and phone number found' 
                        };
                    }
                }
            }
    ...
    const validateStep = useCallback(async (
        ...
        const validateFn = validationFunctions[stepName as keyof typeof validationFunctions];
        if (!validateFn) {
        ...
        if (stepName === 'basic' && enableDuplicateCheck && data.name) {
            ...
            if (businessDuplicateResult.isDuplicate && businessDuplicateResult.matchReason) {
    ...
    useEffect(() => {
        ...
        const runValidation = async () => {
            ...
            try {
                // Validate all steps
                const [basicResult, contactResult, businessResult] = await Promise.all([
    ...
    const getStepValidation = useCallback((stepName: string): StepValidationResult => {
        return validationState.validationResults[stepName] || { 
            isValid: true, 
            errors: [], 
            warnings: [] 
        };
    ...
    const validateStepManually = useCallback(async (stepName: string): Promise<StepValidationResult> => {
