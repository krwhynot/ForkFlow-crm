...
export const validateTextInput = (
    ...
    if (
        !options.allowHtml &&
        (SECURITY_PATTERNS.XSS_SCRIPT.test(input) ||
            SECURITY_PATTERNS.XSS_ONERROR.test(input))
    ) {
    ...
    if (
        SECURITY_PATTERNS.COMMAND_INJECTION.test(input) &&
...
export const sanitizeInput = (
    input: any,
    type: 'text' | 'email' | 'url' | 'filename' = 'text'
): string => {
