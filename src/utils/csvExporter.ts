...
export const CSV_COLUMN_CONFIGS = {
    organizations: [
        ...
        { key: 'priority', header: 'Priority', transform: (p: any) => p?.label || '' },
        { key: 'segment', header: 'Segment', transform: (s: any) => s?.label || '' },
        { key: 'distributor', header: 'Distributor', transform: (d: any) => d?.label || '' },
        ...
        { key: 'lastContactDate', header: 'Last Contact Date', transform: (date: string) => date ? new Date(date).toLocaleDateString() : '' },
        { key: 'createdAt', header: 'Created At', transform: (date: string) => new Date(date).toLocaleDateString() },
        { key: 'updatedAt', header: 'Updated At', transform: (date: string) => new Date(date).toLocaleDateString() },
    ] as CSVColumn[],
    ...
    contacts: [
        ...
        { key: 'phone', header: 'Phone' },
        { key: 'organization', header: 'Organization', transform: (org: any) => org?.name || '' },
        { key: 'role', header: 'Role', transform: (role: any) => role?.label || '' },
        { key: 'influenceLevel', header: 'Influence Level', transform: (level: any) => level?.label || '' },
        { key: 'decisionRole', header: 'Decision Role', transform: (role: any) => role?.label || '' },
        { key: 'linkedInUrl', header: 'LinkedIn URL' },
        { key: 'isPrimary', header: 'Primary Contact', transform: (isPrimary: boolean) => isPrimary ? 'Yes' : 'No' },
        { key: 'notes', header: 'Notes' },
        { key: 'lastInteractionDate', header: 'Last Interaction', transform: (date: string) => date ? new Date(date).toLocaleDateString() : '' },
    ...
    interactions: [
        { key: 'id', header: 'ID' },
        { key: 'organization', header: 'Organization', transform: (org: any) => org?.name || '' },
        { key: 'contact', header: 'Contact', transform: (contact: any) => contact ? `${contact.firstName} ${contact.lastName}` : '' },
        { key: 'type', header: 'Type', transform: (type: any) => type?.label || '' },
        { key: 'subject', header: 'Subject' },
        { key: 'description', header: 'Description' },
        { key: 'scheduledDate', header: 'Scheduled Date', transform: (date: string) => date ? new Date(date).toLocaleDateString() : '' },
        { key: 'completedDate', header: 'Completed Date', transform: (date: string) => date ? new Date(date).toLocaleDateString() : '' },
        { key: 'isCompleted', header: 'Completed', transform: (completed: boolean) => completed ? 'Yes' : 'No' },
        { key: 'duration', header: 'Duration (minutes)' },
        { key: 'outcome', header: 'Outcome' },
        { key: 'followUpRequired', header: 'Follow-up Required', transform: (required: boolean) => required ? 'Yes' : 'No' },
        { key: 'followUpDate', header: 'Follow-up Date', transform: (date: string) => date ? new Date(date).toLocaleDateString() : '' },
        { key: 'latitude', header: 'Latitude' },
        { key: 'longitude', header: 'Longitude' },
    ...
    products: [
        ...
        { key: 'principal', header: 'Principal', transform: (principal: any) => principal?.label || '' },
        { key: 'category', header: 'Category' },
        { key: 'description', header: 'Description' },
        { key: 'sku', header: 'SKU' },
        { key: 'unitOfMeasure', header: 'Unit of Measure' },
        { key: 'packageSize', header: 'Package Size' },
        { key: 'price', header: 'Price', transform: (price: number) => price ? `$${price.toFixed(2)}` : '' },
        { key: 'active', header: 'Active', transform: (active: boolean) => active ? 'Yes' : 'No' },
        { key: 'createdAt', header: 'Created At', transform: (date: string) => new Date(date).toLocaleDateString() },
    ] as CSVColumn[],
    ...
    opportunities: [
        ...
        { key: 'organization', header: 'Organization', transform: (org: any) => org?.name || '' },
        { key: 'contact', header: 'Contact', transform: (contact: any) => contact ? `${contact.firstName} ${contact.lastName}` : '' },
        { key: 'product', header: 'Product', transform: (product: any) => product?.name || '' },
        { key: 'stage', header: 'Stage', transform: (stage: any) => stage?.label || '' },
        { key: 'amount', header: 'Amount', transform: (amount: number) => amount ? `$${amount.toLocaleString()}` : '' },
        { key: 'probability', header: 'Probability %' },
        { key: 'expectedCloseDate', header: 'Expected Close Date', transform: (date: string) => date ? new Date(date).toLocaleDateString() : '' },
        { key: 'description', header: 'Description' },
        { key: 'createdAt', header: 'Created At', transform: (date: string) => new Date(date).toLocaleDateString() },
        { key: 'updatedAt', header: 'Updated At', transform: (date: string) => new Date(date).toLocaleDateString() },
    ] as CSVColumn[],
 ...
 */
export function exportOrganizationsCSV(organizations: any[], filename?: string): void {
    ...
    downloadCSV(csvContent, filename || generateCSVFilename('organizations-export'));
}
 ...
 */
export function exportInteractionsCSV(interactions: any[], filename?: string): void {
    ...
    downloadCSV(csvContent, filename || generateCSVFilename('interactions-export'));
}
 ...
 */
export function exportOpportunitiesCSV(opportunities: any[], filename?: string): void {
    ...
    downloadCSV(csvContent, filename || generateCSVFilename('opportunities-export'));
}
