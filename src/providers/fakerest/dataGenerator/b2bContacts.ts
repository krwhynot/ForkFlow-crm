...
export const generateB2BContacts = (db: Db): Contact[] => {
    ...
    const roleSettings =
        db.settings?.filter(s => s.category === 'role' && s.active) || [];
    const influenceSettings =
        db.settings?.filter(s => s.category === 'influence' && s.active) || [];
    const decisionSettings =
        db.settings?.filter(s => s.category === 'decision' && s.active) || [];
        ...
        for (let i = 0; i < contactCount; i++) {
            ...
            const roleId =
                roleSettings.length > 0
                    ? faker.helpers.arrayElement(roleSettings).id
                    : undefined;
            const influenceLevelId =
                weightedBoolean(0.8) && influenceSettings.length > 0
                    ? faker.helpers.arrayElement(influenceSettings).id
                    : undefined;
            const decisionRoleId =
                weightedBoolean(0.7) && decisionSettings.length > 0
                    ? faker.helpers.arrayElement(decisionSettings).id
                    : undefined;
            ...
            const phone = weightedBoolean(0.8)
                ? faker.phone.number()
                : undefined;
            ...
            const influence = influenceSettings.find(
                i => i.id === influenceLevelId
            );
            const decision = decisionSettings.find(
                d => d.id === decisionRoleId
            );
            ...
            const noteElements = [
                ...
                influence
                    ? `Has ${influence.label.toLowerCase()} influence in decision making.`
                    : '',
                decision
                    ? `Serves as ${decision.label.toLowerCase()} in the buying process.`
                    : '',
