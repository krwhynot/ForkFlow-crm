...
it('should return null if an empty string is provided', async () => {
    const email = '';
    const record: Partial<Contact> = { email };
...
// This test is no longer relevant since we only support single email now
// Keeping it commented out in case multiple emails support is added later
