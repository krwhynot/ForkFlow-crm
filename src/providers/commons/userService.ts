import { DataProvider } from 'react-admin';
import { Sale } from '../../types';

export const DEFAULT_USER = {
    id: 0,
    first_name: 'Jane',
    last_name: 'Doe',
    email: 'janedoe@atomic.dev',
    password: 'demo',
    administrator: true,
    avatar: {
        src: 'data:image/jpeg;base64,',
    },
} as const;

/**
 * UserService encapsulates user data access logic to break circular dependency
 * between authProvider and dataProvider.
 */
export class UserService {
    private dataProvider: DataProvider;

    constructor(dataProvider: DataProvider) {
        this.dataProvider = dataProvider;
    }

    /**
     * Get user by email for authentication validation
     */
    async getUser(email: string): Promise<Sale> {
        try {
            const sales = await this.dataProvider.getList('sales', {
                pagination: { page: 1, perPage: 200 },
                sort: { field: 'name', order: 'ASC' },
            });

            if (!sales.data.length) {
                return { ...DEFAULT_USER };
            }

            const user = sales.data.find(sale => sale.email === email);
            if (!user || user.disabled) {
                return { ...DEFAULT_USER };
            }

            return user;
        } catch (error) {
            console.error('UserService.getUser error:', error);
            return { ...DEFAULT_USER };
        }
    }

    /**
     * Validate user credentials
     */
    async validateUser(email: string, password?: string): Promise<{ valid: boolean; user?: Sale }> {
        try {
            const user = await this.getUser(email);
            
            // For demo purposes, any valid user is considered authenticated
            // In production, implement proper password validation
            if (user && user.id !== DEFAULT_USER.id) {
                return { valid: true, user };
            }

            return { valid: false };
        } catch (error) {
            console.error('UserService.validateUser error:', error);
            return { valid: false };
        }
    }

    /**
     * Check if user exists in the system
     */
    async userExists(email: string): Promise<boolean> {
        try {
            const user = await this.getUser(email);
            return user.id !== DEFAULT_USER.id;
        } catch (error) {
            console.error('UserService.userExists error:', error);
            return false;
        }
    }

    /**
     * Get user by ID
     */
    async getUserById(id: string | number): Promise<Sale | null> {
        try {
            const response = await this.dataProvider.getOne('sales', { id });
            return response.data || null;
        } catch (error) {
            console.error('UserService.getUserById error:', error);
            return null;
        }
    }
}

// Export a default instance factory function
export const createUserService = (dataProvider: DataProvider): UserService => {
    return new UserService(dataProvider);
};