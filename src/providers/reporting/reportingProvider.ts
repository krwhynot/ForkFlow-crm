...
import { arrayToCSV, generateCSVFilename, CSV_COLUMN_CONFIGS } from '../../utils/csvExporter';
    ...
    return {
        ...
        async getDashboardReport(): Promise<DashboardSummary> {
            try {
                ...
                const [organizationsResult, contactsResult, interactionsResult, dealsResult, settingsResult] = await Promise.all([
                ...
                const dailyInteractions = interactions.filter(i => new Date(i.createdAt) >= oneDayAgo).length;
                const weeklyInteractions = interactions.filter(i => new Date(i.createdAt) >= oneWeekAgo).length;
                const monthlyInteractions = interactions.filter(i => new Date(i.createdAt) >= oneMonthAgo).length;
        ...
        async getInteractionReport(
            ...
            try {
                ...
                if (startDate) {
                    interactions = interactions.filter(i => new Date(i.createdAt) >= new Date(startDate));
                }
                if (endDate) {
                    interactions = interactions.filter(i => new Date(i.createdAt) <= new Date(endDate));
                }

                // Apply additional filters
                if (filters) {
                    if (filters.organizationId) {
                        interactions = interactions.filter(i => i.organizationId === filters.organizationId);
                    }
                    if (filters.typeId) {
                        interactions = interactions.filter(i => i.typeId === filters.typeId);
                    }
                }

                // Get interaction types from settings
                const interactionTypes = settings.filter(s => s.category === 'interaction_type');
                const principals = settings.filter(s => s.category === 'principal');
                const segments = settings.filter(s => s.category === 'segment');

                // Calculate metrics by type
                const byType = interactionTypes.map(type => {
                    const count = interactions.filter(i => i.typeId === type.id).length;
                ...
                for (let i = 29; i >= 0; i--) {
                    ...
                    timeline.push({
                        ...
                        completed: dayInteractions.filter(i => i.isCompleted).length,
                    });
                }
         ...
         */
        async getOrganizationsNeedingVisit(): Promise<OrganizationNeedsVisit[]> {
            try {
                // Fetch organizations, contacts, interactions, and settings
                const [organizationsResult, contactsResult, interactionsResult, settingsResult] = await Promise.all([
                    baseDataProvider.getList('organizations', {
                        pagination: { page: 1, perPage: 10000 },
                        sort: { field: 'id', order: 'ASC' },
                        filter: {},
                ...
                const prioritySettings = settings.filter(s => s.category === 'priority');
                const segmentSettings = settings.filter(s => s.category === 'segment');
                ...
                organizations.forEach(org => {
                    ...
                    const lastInteraction = orgInteractions.length > 0 ? orgInteractions[0] : null;
                    const lastContactDate = lastInteraction ? lastInteraction.createdAt : null;
                    ...
                    if (daysSinceContact >= 30) {
                        ...
                        const priority = prioritySettings.find(p => p.id === org.priorityId);
                        const segment = segmentSettings.find(s => s.id === org.segmentId);
                ...
                return needsVisit.sort((a, b) => b.urgencyScore - a.urgencyScore);
            } catch (error) {
                console.error('Error finding organizations needing visits:', error);
         ...
         */
        async exportOrganizations(filters?: Record<string, any>): Promise<CSVExportData> {
            try {
                ...
                const priorityMap = new Map(settings.filter(s => s.category === 'priority').map(s => [s.id, s]));
                const segmentMap = new Map(settings.filter(s => s.category === 'segment').map(s => [s.id, s]));
                const distributorMap = new Map(settings.filter(s => s.category === 'distributor').map(s => [s.id, s]));

                // Enrich organizations with setting objects for CSV export
                const enrichedOrganizations = organizations.map(org => ({
                    ...org,
                    priority: org.priorityId ? priorityMap.get(org.priorityId) : undefined,
                    segment: org.segmentId ? segmentMap.get(org.segmentId) : undefined,
                    distributor: org.distributorId ? distributorMap.get(org.distributorId) : undefined,
                }));
         ...
         */
        async exportInteractions(filters?: Record<string, any>): Promise<CSVExportData> {
            try {
                const [interactionsResult, organizationsResult, contactsResult, settingsResult] = await Promise.all([
                    baseDataProvider.getList('interactions', {
                        pagination: { page: 1, perPage: 10000 },
                        sort: { field: 'createdAt', order: 'DESC' },
                        filter: filters || {},
                ...
                const typeMap = new Map(settings.filter(s => s.category === 'interaction_type').map(s => [s.id, s]));

                // Enrich interactions with related objects for CSV export
                const enrichedInteractions = interactions.map(interaction => ({
                    ...interaction,
                    organization: organizationMap.get(interaction.organizationId),
