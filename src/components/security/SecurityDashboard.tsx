...
import { getAuditStatistics, getLocalAuditEvents, AuditEvent } from '../../utils/auditLogging';
        ...
        <Box sx={{ p: isMobile ? 1 : 2 }}>
                ...
                <Grid item xs={12} md={3}>
                    <Card>
                        <CardContent sx={{ textAlign: 'center' }}>
                            <Typography variant="h3" color={securityScore >= 80 ? 'success.main' : securityScore >= 60 ? 'warning.main' : 'error.main'}>
                                {securityScore}
                            </Typography>
            ...
            {!metrics.httpsStatus.isSecure && (
                <Alert severity="error" sx={{ mb: 2 }}>
                    <strong>HTTPS Not Enabled:</strong> Your connection is not secure. Enable HTTPS for production use.
                </Alert>
            )}
            {metrics.auditStats.recentCriticalEvents > 0 && (
                <Alert severity="warning" sx={{ mb: 2 }}>
                    <strong>Critical Security Events:</strong> {metrics.auditStats.recentCriticalEvents} critical security events detected in the last 24 hours.
                </Alert>
            )}

            {/* Detailed Security Sections */}
            <Accordion expanded={expandedSection === 'overview'} onChange={(_, isExpanded) => setExpandedSection(isExpanded ? 'overview' : false)}>
                ...
                <AccordionDetails>
                    <Grid container spacing={3}>
                        <Grid item xs={12} md={6}>
                            <Typography variant="subtitle1" gutterBottom>HTTPS Status</Typography>
                            <List dense>
                                <ListItem>
                                    <ListItemIcon>
                                        {metrics.httpsStatus.isSecure ? <CheckIcon color="success" /> : <ErrorIcon color="error" />}
                                    </ListItemIcon>
                                    <ListItemText primary="Secure Connection" secondary={metrics.httpsStatus.isSecure ? 'HTTPS Enabled' : 'HTTP Only'} />
                                </ListItem>
                                <ListItem>
                                    <ListItemIcon>
                                        {metrics.httpsStatus.hasCSP ? <CheckIcon color="success" /> : <WarningIcon color="warning" />}
                                    </ListItemIcon>
                                    <ListItemText primary="Content Security Policy" secondary={metrics.httpsStatus.hasCSP ? 'Configured' : 'Not Set'} />
                                </ListItem>
                                <ListItem>
                                    <ListItemIcon>
                                        {metrics.httpsStatus.hasHSTS ? <CheckIcon color="success" /> : <WarningIcon color="warning" />}
                                    </ListItemIcon>
                                    <ListItemText primary="Strict Transport Security" secondary={metrics.httpsStatus.hasHSTS ? 'Enabled' : 'Disabled'} />
                                </ListItem>
                            </List>
                        </Grid>
                        <Grid item xs={12} md={6}>
                            ...
                            <List dense>
                                <ListItem>
                                    <ListItemIcon>
                                        {metrics.sessionInfo.isValid ? <CheckIcon color="success" /> : <ErrorIcon color="error" />}
                                    </ListItemIcon>
                                    <ListItemText primary="Session Valid" secondary={metrics.sessionInfo.isValid ? 'Active Session' : 'No Session'} />
                                </ListItem>
                                <ListItem>
                                    <ListItemIcon>
                                        {metrics.offlineStatus.hasCredentials ? <CheckIcon color="success" /> : <WarningIcon color="warning" />}
                                    </ListItemIcon>
                                    <ListItemText primary="Offline Authentication" secondary={metrics.offlineStatus.hasCredentials ? 'Available' : 'Not Available'} />
                                </ListItem>
                            </List>
                        </Grid>
                    </Grid>
            ...
            <Accordion expanded={expandedSection === 'audit'} onChange={(_, isExpanded) => setExpandedSection(isExpanded ? 'audit' : false)}>
                <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                ...
                <AccordionDetails>
                    <Box sx={{ mb: 2 }}>
                        <Typography variant="subtitle2" gutterBottom>Events by Category</Typography>
                        <Grid container spacing={1}>
                            {Object.entries(metrics.auditStats.eventsByCategory).map(([category, count]) => (
                    ...
                    <Box sx={{ mb: 2 }}>
                        <Typography variant="subtitle2" gutterBottom>Events by Severity</Typography>
                        <Grid container spacing={1}>
                            {Object.entries(metrics.auditStats.eventsBySeverity).map(([severity, count]) => (
                                    ...
                                    <TableRow key={event.id}>
                                        ...
                                        <TableCell>
                                            <Chip 
                                                label={event.outcome} 
                                                size="small"
                                                color={event.outcome === 'success' ? 'success' : event.outcome === 'failure' ? 'error' : 'warning'}
                                            />
                                        </TableCell>
                                    </TableRow>
                                ))}
            ...
            <Accordion expanded={expandedSection === 'recommendations'} onChange={(_, isExpanded) => setExpandedSection(isExpanded ? 'recommendations' : false)}>
                <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                    <BugIcon sx={{ mr: 1 }} />
                ...
                <AccordionDetails>
                    <List>
                        {metrics.httpsStatus.recommendations.map((rec, index) => (
                            <ListItem key={index}>
                                <ListItemIcon>
