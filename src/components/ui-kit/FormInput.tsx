...
export const FormInput: React.FC<FormInputProps> = ({
    ...
    const getInputClasses = () => {
        ...
        if (error) {
            classes = cn(classes, 'border-red-300 focus:border-red-500 focus:ring-red-500');
        } else if (warning) {
            classes = cn(classes, 'border-yellow-300 focus:border-yellow-500 focus:ring-yellow-500');
        } else {
            classes = cn(classes, 'border-gray-300 focus:border-blue-500 focus:ring-blue-500');
        }

        return cn(classes, inputClassName);
    };
        ...
        <div className={cn('space-y-2', className)}>
            ...
            <div className="space-y-1">
                {error && (
                    <div 
                        id={`${inputId}-error`}
                        className="flex items-start space-x-1 text-red-600"
                    >
                ...
                {warning && !error && (
                    <div 
                        id={`${inputId}-warning`}
                        className="flex items-start space-x-1 text-yellow-600"
                    >
